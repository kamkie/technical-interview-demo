...
- `src/manualTests/http/examples/authentication.http` shows the bootstrap, provider-login, authenticated-session refresh, and logout flow
- `src/manualTests/http/examples/user-account-controller.http` shows one authenticated unsafe write using the documented `XSRF-TOKEN` cookie plus `X-XSRF-TOKEN` header handshake
...
5. Repeat the login flow and verify `GET /api/account` with the captured `technical-interview-demo-session` cookie from `src/manualTests/http/examples/authentication.http`.
