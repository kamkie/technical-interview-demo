...
## UI Integration And Upgrade

Use the published [upgrade guide](src/docs/asciidoc/upgrade-1x-to-2-0.adoc) for the full `1.x` to `2.0` migration narrative. The packaged docs serve the same page at `/docs/upgrade-1x-to-2-0.html`.

Steady-state first-party UI integration stays small:

- keep one public origin and expose only `/api/**` from this application; the checked-in [edge reference](k8s/edge/public-api-ingress.yaml) shows that boundary
- call `GET /api/session` on initial load to discover `loginProviders[]`, logout metadata, and the current CSRF cookie/header contract
- start interactive login from the returned relative `authorizationPath` instead of hard-coding provider routes into the UI
- keep `GET /api/account` for authenticated persisted-profile reads, not anonymous bootstrap
...
- mirror the readable `XSRF-TOKEN` cookie into the `X-XSRF-TOKEN` request header on unsafe `/api/**` requests such as `PUT /api/account/language`

For runnable reviewer examples, use `src/test/resources/http/authentication.http` for the bootstrap/login/logout flow and `src/test/resources/http/user-account-controller.http` for the authenticated CSRF-backed write sequence.
...
- pull requests to `main` and pushes to `main` run the `CI` workflow, which executes `./gradlew build` and `./gradlew externalSmokeTest`; pushes to `main` also submit the Gradle dependency snapshot generated by that same build so GitHub dependency graph data stays aligned with the CI-validated dependency set
...
- `externalSmokeTest` now verifies the packaged docs HTML, OpenAPI JSON/YAML endpoints, the published CSRF and abuse-protection posture on `GET /`, JDBC-backed `GET /api/session` bootstrap with `XSRF-TOKEN` issuance, and one authenticated `PUT /api/account/language` write in addition to the existing public/readiness smoke checks
- the scheduled `Post-Deploy Smoke` workflow runs `./gradlew scheduledExternalCheck` every six hours and on manual dispatch, using `EXTERNAL_CHECK_BASE_URL` plus optional JDBC secrets for deeper JDBC-backed session/bootstrap/write and Flyway checks
...
- when JDBC access is configured, deployed smoke also proves `GET /api/session`, readable `XSRF-TOKEN` bootstrap, one authenticated `PUT /api/account/language`, and persisted authenticated account access; without that JDBC access the workflow stays HTTP-only by design
...
- deployment artifacts are provided as:
  ...
  - a checked-in `/api/**` public-edge reference under `k8s/edge/public-api-ingress.yaml`
...
The checked-in edge reference at `k8s/edge/public-api-ingress.yaml` shows the intended `/api/**`-only public route shape for that topology.
...
3. confirm the drill proves readiness, `build.version`, `git.shortCommitId`, `prod` profile activation, JDBC session storage, the `15m` session timeout, `csrfEnabled=true`, `csrfCookieName=XSRF-TOKEN`, `csrfHeaderName=X-XSRF-TOKEN`, edge-owned abuse-protection metadata, Flyway history, `GET /api/session` bootstrap with readable `XSRF-TOKEN`, one authenticated `PUT /api/account/language`, and a persisted authenticated `GET /api/account` check against the restored database
4. keep the restore evidence with the release notes for every `restore-sensitive` rollout

Use the raw manifests under `k8s/` when you want explicit repo-owned YAML. That includes the illustrative public-edge reference under `k8s/edge/` alongside the base deployment and monitoring assets. Use the Helm chart under `helm/technical-interview-demo` when you want the same deployment contract packaged behind values files.
