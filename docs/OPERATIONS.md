...
- [UI Integration References](#ui-integration-references)
- [CI And Release Workflow Expectations](#ci-and-release-workflow-expectations)
...
- repo-owned infrastructure assets under [infra/](../infra/)
...
- vendor-neutral Kubernetes manifests under [infra/k8s/](../infra/k8s/)
- checked-in `/api/**` public-edge reference assets under [infra/k8s/edge/](../infra/k8s/edge/)
- a matching Helm chart under [infra/helm/technical-interview-demo/](../infra/helm/technical-interview-demo/)
- monitoring and alerting assets for Prometheus, Grafana, and Alertmanager under [infra/monitoring/](../infra/monitoring/)

## UI Integration References

Use these checked-in artifacts together when wiring or reviewing the separate first-party UI:

- [authentication.http](../src/manualTests/http/examples/authentication.http) is a convenience script for the bootstrap, provider-login, authenticated-session refresh, and logout flow
- [user-account-controller.http](../src/manualTests/http/examples/user-account-controller.http) is a convenience script for one authenticated unsafe write using the documented `XSRF-TOKEN` cookie plus `X-XSRF-TOKEN` header handshake
- [public-api-ingress.yaml](../infra/k8s/edge/public-api-ingress.yaml) and [infra/k8s/edge/README.md](../infra/k8s/edge/README.md) are the checked-in `/api/**` public-edge reference assets for the one-public-origin deployment model
- [FRONTEND_AI_CONTRACT.md](FRONTEND_AI_CONTRACT.md) is the generated backend contract summary for separate first-party frontend AI agents
...
The current workflow pins `cosign v3.0.5` for signing and immediate post-sign verification.
Use the same disposable Cosign image for manual checks unless [release.yml](../.github/workflows/release.yml) intentionally changes:
...
The full `build` task includes the Docker image build.
If you only want the Gradle artifacts and checks, run `./build.ps1 -FullBuild build -x dockerBuild`.
...
The repository CI performs a production-like container smoke test through the Gradle `externalSmokeTest` task after `./build.ps1 -FullBuild build`.
...
- the Grafana dashboard and Prometheus rules under [infra/monitoring/](../infra/monitoring/) and [infra/k8s/monitoring/](../infra/k8s/monitoring/) assume those metrics stay visible
...
Restore drill is recommended each release cycle:
...
The raw Kubernetes baseline lives under [infra/k8s/base/](../infra/k8s/base/).
The local demo overlay lives under [infra/k8s/overlays/local/](../infra/k8s/overlays/local/).
The illustrative `/api/**` public-edge reference lives under [infra/k8s/edge/](../infra/k8s/edge/) and is kept separate from the base package because it is a reviewed example, not a turnkey ingress install.

Secret handling:

- [secret-example.yaml](../infra/k8s/base/secret-example.yaml) is an example only and is not included in the base Kustomize package
...
- [horizontalpodautoscaler.yaml](../infra/k8s/base/horizontalpodautoscaler.yaml) scales the app between 2 and 6 replicas on CPU and memory utilization
- [poddisruptionbudget.yaml](../infra/k8s/base/poddisruptionbudget.yaml) requires at least one pod to remain available during voluntary disruptions
...
3. Make sure a PostgreSQL service named `postgres` exists in the `technical-interview-demo` namespace, or patch `DATABASE_HOST` in [patch-configmap.yaml](../infra/k8s/overlays/local/patch-configmap.yaml).
4. Create the deployment secret from [secret-example.yaml](../infra/k8s/base/secret-example.yaml) or with `kubectl create secret generic`.
...
Use the raw manifests when you want to review or patch explicit YAML checked into the repo.
Use the Helm chart when you want release-style installs driven by values files.
...
- [values.yaml](../infra/helm/technical-interview-demo/values.yaml) enables autoscaling and a pod disruption budget by default for deployment-style installs
- [values-local.yaml](../infra/helm/technical-interview-demo/values-local.yaml) matches the local overlay assumptions: single replica, local image tag, `postgres` service host, non-secure session cookie for HTTP testing, and autoscaling disabled
...
The repository does not vendor the monitoring stack itself.
Instead, it assumes the upstream `kube-prometheus-stack` Helm chart and adds repo-owned assets for the app scrape config, alert rules, Grafana dashboard, and Alertmanager examples.
...
- [servicemonitor.yaml](../infra/k8s/monitoring/servicemonitor.yaml) scrapes `GET /actuator/prometheus` from inside the cluster as a trusted deployment concern, not an internet-public endpoint
- [prometheus-rule.yaml](../infra/k8s/monitoring/prometheus-rule.yaml) adds alerts for target-down, readiness failures, repeated restarts, elevated auth failures, session-backed account 5xxs, database pool saturation, database connection timeouts, Flyway-style startup crash loops, and elevated 5xx rates
- [technical-interview-demo.json](../infra/monitoring/grafana/dashboards/technical-interview-demo.json) adds starter panels for JVM memory, CPU, request throughput/latency, cache events, domain totals, authentication failures, session-backed account health, database pool saturation, and Flyway migration visibility
- [config-example.yaml](../infra/monitoring/alertmanager/config-example.yaml) is a starting Alertmanager route/receiver config that you should adapt before using real notifications
...
- [infra/k8s/log-forwarding/fluent-bit/](../infra/k8s/log-forwarding/fluent-bit/) is an optional forwarder example that recombines multiline Java stack traces before shipping logs to a centralized HTTP endpoint
...
Before applying to a real cluster, update [configmap.yaml](../infra/k8s/log-forwarding/fluent-bit/configmap.yaml) with your destination host, port, URI, and TLS policy.
...
Use it when you want to exercise protected write endpoints from a browser or deployed same-site UI.
...
Unsafe `/api/**` writes also require the same-site CSRF handshake:
...
5. Repeat the login flow and verify `GET /api/account` with the captured `technical-interview-demo-session` cookie. The convenience script at [authentication.http](../src/manualTests/http/examples/authentication.http) shows the request shape.
...
1. Confirm the monitoring stack was installed with [kube-prometheus-stack-values.yaml](../infra/monitoring/kube-prometheus-stack-values.yaml).
