...
public class AuthenticationRedirectSimulation extends Simulation {
    ...
    private final HttpProtocolBuilder httpProtocol = http.baseUrl(PerformanceEnvironment.baseUrl())
            .disableFollowRedirect()
            .userAgentHeader("gatling-phase-9-auth-redirect");

    private final ScenarioBuilder scenarioBuilder = scenario("oauth-redirect-start")
            .exec(http("oauth2-github-redirect")
...

    {
        setUp(scenarioBuilder.injectOpen(
                        atOnceUsers(3),
                        rampUsersPerSec(1).to(4).during(Duration.ofSeconds(15)),
                        constantUsersPerSec(4).during(Duration.ofSeconds(15))))
                .protocols(httpProtocol)
                .assertions(
                        global().successfulRequests().percent().gte(99.0),
                        details("oauth2-github-redirect")
                                .responseTime()
                                .percentile3()
                                .lt(700));
    }
