 ...
 */
@SuiteName(value = "06-session-and-account", requires = {"01-public-overview-and-docs"}, requiresAdminIdentity = true)
public class Suite06SessionAndAccount extends SuiteBase {
    ...
    void session_isAuthenticated() {
        Response response = http().send("GET", "/api/session", http().asAdmin(), 200, Optional.of("admin /api/session"));
    ...
    void account_returnsAdminIdentity() {
        Response response = http().send("GET", "/api/account", http().asAdmin(), 200, Optional.of("admin /api/account"));
    ...
    void changePreferredLanguageToPl() {
        ...
        int status = http().send(
                        ...
                        http().asAdmin().contentType(ContentType.JSON).body("{\"preferredLanguage\":\"" + target + "\"}"),
                        null,
    ...
    void restorePreferredLanguage() {
        ...
        Response response = http().send(
                "PUT",
                "/api/account/language",
                http().asAdmin().contentType(ContentType.JSON).body("{\"preferredLanguage\":\"" + originalPreferredLanguage + "\"}"),
                null,
                Optional.of("restore original preferred language"));
        if (response.statusCode() != 200 && response.statusCode() != 204) {
            leftover("preferredLanguage was not restored to " + originalPreferredLanguage
                    + "; manual restore required");
            throw new AssertionError(
                    "Failed to restore preferredLanguage; HTTP " + response.statusCode());
        }
    }
}
