 ...
 */
@SuiteName(value = "02-actuator-and-observability", requires = {"01-public-overview-and-docs"})
public class Suite02ActuatorAndObservability extends SuiteBase {
    ...
    void livenessAndReadiness_areUp() {
        String liveness =
                http().send("GET", "/actuator/health/liveness", http().anonymous(), 200, Optional.of("liveness"))
                        .jsonPath()
                        .getString("status");
        String readiness =
                http().send("GET", "/actuator/health/readiness", http().anonymous(), 200, Optional.of("readiness"))
                        .jsonPath()
                        .getString("status");
    ...
    void prometheusScrape_servesNonEmptyBody() {
        String body = http()
                .send("GET", "/actuator/prometheus", http().anonymous(), 200, Optional.of("prometheus scrape"))
                .asString();
