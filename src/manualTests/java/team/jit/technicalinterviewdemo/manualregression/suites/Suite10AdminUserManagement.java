...
public class Suite10AdminUserManagement extends SuiteBase {
    ...
    void grantAndRestoreRolesForRegularUser() {
        ...
        originalRoles = rolesForUser(users, userId).orElse(null);
        ...
        String body = "{\"roles\":" + toJsonStringArray(grantedSet) + ",\"reason\":\"Manual regression round-trip for "
                + runTag() + "\"}";
    ...
    private static Optional<List<String>> rolesForUser(List<?> users, String userId) {
        for (Object candidate : users) {
            if (candidate instanceof Map<?, ?> user
                    && String.valueOf(user.get("id")).equals(userId)) {
                Object roles = user.get("roles");
                if (roles instanceof List<?> roleList) {
                    return Optional.of(roleList.stream().map(String::valueOf).toList());
                }
            }
        }
