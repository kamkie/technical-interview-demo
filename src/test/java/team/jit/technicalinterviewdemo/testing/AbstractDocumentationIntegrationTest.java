...
public abstract class AbstractDocumentationIntegrationTest extends AbstractMockMvcIntegrationTest {
    ...
    protected HeaderDescriptor[] commonResponseHeaders(HeaderDescriptor... additionalHeaders) {
        List<HeaderDescriptor> headers = new ArrayList<>(List.of(
                ...
                headerWithName("Permissions-Policy")
                        .description(
                                "Response hardening header that disables geolocation, microphone, and camera browser"
                                        + " features."),
                headerWithName("Strict-Transport-Security")
                        .optional()
                        .description("Present on secure responses in `prod` only to enforce HTTPS caching."),
