...
public abstract class AbstractDocumentationIntegrationTest extends AbstractMockMvcIntegrationTest {
    ...
    protected FieldDescriptor[] problemResponseFields() {
        return new FieldDescriptor[]{fieldWithPath("title").description("Problem title."), fieldWithPath("status").description("HTTP status code."), fieldWithPath("detail").description("Technical problem detail kept stable for debugging and logs."), fieldWithPath("messageKey").description("Stable localization key for the error type."), fieldWithPath("message").description("Localized end-user message resolved from the request language."), fieldWithPath("language").description("Two-letter ISO 639-1 language code actually used for the localized message.")
        };
    }

    protected FieldDescriptor[] problemResponseFieldsWithFieldErrors() {
        return new FieldDescriptor[]{fieldWithPath("title").description("Problem title."), fieldWithPath("status").description("HTTP status code."), fieldWithPath("detail").description("Technical problem detail kept stable for debugging and logs."), fieldWithPath("messageKey").description("Stable localization key for the error type."), fieldWithPath("message").description("Localized end-user message resolved from the request language."), fieldWithPath("language").description("Two-letter ISO 639-1 language code actually used for the localized message."), subsectionWithPath("fieldErrors").description("Validation errors keyed by request field name.")
        };
    }
}
