...
class ApiErrorHandlingIntegrationTests extends AbstractBookCatalogMockMvcIntegrationTest {
    ...
    void createBookWithUnsupportedMediaTypeReturnsUnsupportedMediaType() throws Exception {
        ...
        mockMvc.perform(post("/api/books").with(browserSession.unsafeWrite()).contentType(MediaType.TEXT_PLAIN).content("not-json")).andExpect(status().isUnsupportedMediaType()).andExpect(jsonPath("$.title").value("Unsupported Media Type"));
    }

    @Test
    void getBookByInvalidIdReturnsBadRequest() throws Exception {
        mockMvc.perform(get("/api/books/{id}", "abc")).andExpect(status().isBadRequest()).andExpect(jsonPath("$.title").value("Invalid Parameter")).andExpect(jsonPath("$.detail").value("Parameter 'id' value 'abc' is invalid."));
    }

    @Test
    void unsupportedMethodReturnsMethodNotAllowed() throws Exception {
        mockMvc.perform(patch("/api/books")).andExpect(status().isMethodNotAllowed()).andExpect(jsonPath("$.title").value("Method Not Allowed")).andExpect(jsonPath("$.detail").value("HTTP method 'PATCH' is not supported for this endpoint."));
    }

    @Test
    void missingResourceReturnsNotFoundProblemDetail() throws Exception {
        mockMvc.perform(get("/api/missing")).andExpect(status().isNotFound()).andExpect(jsonPath("$.title").value("Resource Not Found")).andExpect(jsonPath("$.detail").value("Resource 'api/missing' was not found.")).andExpect(jsonPath("$.messageKey").value("error.request.resource_not_found")).andExpect(jsonPath("$.message").value("The requested resource was not found.")).andExpect(jsonPath("$.language").value("en"));
    }
