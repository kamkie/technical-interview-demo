...
class SensitiveDataSanitizerTests {
    ...
    void sanitizeParametersEscapesValuesAndRedactsSensitiveNames() {
        Map<String, Object> sanitized = SensitiveDataSanitizer.sanitizeParameters(Map.of(
                "query", new String[]{"line1\r\nline2"}, "token", new String[]{"secret-value"}, "multi", new String[]{"a", "b\r\nc"}
        ));

        assertThat(sanitized).containsEntry("query", "line1\\r\\nline2").containsEntry("token", SensitiveDataSanitizer.REDACTED).containsEntry("multi", List.of("a", "b\\r\\nc"));
    }
    ...
    void sanitizeContextForLogEscapesNestedValues() {
        ...
        assertThat(sanitized).containsEntry("title", "Invalid\\r\\nTitle").containsEntry("nested", Map.of("detail", "bad\\r\\nvalue"));
    }
}
