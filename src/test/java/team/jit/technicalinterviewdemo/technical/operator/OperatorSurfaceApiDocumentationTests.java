...
class OperatorSurfaceApiDocumentationTests extends AbstractDocumentationIntegrationTest {
    ...
    void documentGetOperatorSurfaceEndpoint() throws Exception {
        mockMvc.perform(get("/api/admin/operator-surface").with(adminOauthUser()))
                ...
                .andDo(documentEndpoint(
                        "operator/get-operator-surface",
                        responseHeaders(commonResponseHeaders()),
                        relaxedResponseFields(
                                fieldWithPath("audit.auditLogEndpoint")
                                        .description("Endpoint path for full audit log pagination."),
                                fieldWithPath("audit.totalEntries").description("Current number of stored audit rows."),
                                fieldWithPath("audit.recentEntries[].id").description("Audit log identifier."),
                                fieldWithPath("audit.recentEntries[].targetType")
                                        .description("Type of audited target."),
                                fieldWithPath("audit.recentEntries[].targetId")
                                        .description("Identifier of the audited target."),
                                fieldWithPath("audit.recentEntries[].action").description("Recorded action."),
                                fieldWithPath("audit.recentEntries[].actorLogin")
                                        .description("Login of the acting user."),
                                fieldWithPath("audit.recentEntries[].summary")
                                        .description("Human-readable audit summary."),
                                subsectionWithPath("audit.recentEntries[].details")
                                        .description("Compact structured audit details for each recent audit entry."),
                                fieldWithPath("audit.recentEntries[].createdAt")
                                        .description("Creation timestamp as a UTC instant."),
                                fieldWithPath("runtime.technicalOverviewEndpoint")
                                        .description("Endpoint path for the full technical overview payload."),
                                subsectionWithPath("runtime.technicalOverview")
                                        .description("Current technical overview payload from `GET /`."),
                                fieldWithPath("operations.actuatorHealthEndpoint")
                                        .description("Endpoint path for aggregate health."),
                                fieldWithPath("operations.actuatorInfoEndpoint")
                                        .description("Endpoint path for build and git metadata."),
                                fieldWithPath("operations.actuatorPrometheusEndpoint")
                                        .description("Endpoint path for trusted deployment metrics scraping."),
                                fieldWithPath("operations.applicationHealthStatus")
                                        .description("Current aggregate health status code."),
                                fieldWithPath("operations.livenessState")
                                        .description("Current Spring Boot liveness state."),
                                fieldWithPath("operations.readinessState")
                                        .description("Current Spring Boot readiness state."))));
    }
    ...
    void documentGetOperatorSurfaceForbiddenError() throws Exception {
        mockMvc.perform(get("/api/admin/operator-surface").with(oauthUser("reader-user")))
                ...
                .andDo(documentEndpoint(
                        "errors/get-operator-surface-forbidden",
                        responseHeaders(commonResponseHeaders()),
