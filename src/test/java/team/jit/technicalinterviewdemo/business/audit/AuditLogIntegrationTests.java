...
class AuditLogIntegrationTests extends AbstractMockMvcIntegrationTest {
    ...
    void setUp() {
        ...
        localizationMessageRepository
                .findByMessageKeyAndLanguage(EXISTING_LOCALIZATION_KEY, "en")
    ...
    void categoryCreateUpdateAndDeleteProduceAuditLogsWithAdminActorAndStructuredDetails() throws Exception {
        ...
        mockMvc.perform(delete("/api/categories/{id}", createdCategory.getId()).with(adminSession.unsafeWrite()))
                .andExpect(status().isNoContent());
    ...
    void logoutProducesAuthenticationAuditLog() throws Exception {
        ...
        mockMvc.perform(post("/api/session/logout").with(readerSession.unsafeWrite()))
                .andExpect(status().isNoContent());
