        ...
        mockMvc.perform(get("/api/admin/users")
                ...
                .andDo(documentEndpoint(
                        ...
                        relaxedResponseFields(
                                ...
                                fieldWithPath("[].email").description("Current email when available from the provider."),
                                fieldWithPath("[].preferredLanguage").description("Preferred two-letter language code, or `null` when unset."),
    ...
    void documentReplaceManagedUserRolesEndpoint() throws Exception {
        mockMvc.perform(put("/api/admin/users/{id}/roles", readerUser.getId())
                ...
                .andDo(documentEndpoint(
                        ...
                        requestBody(),
                        requestFields(
                                fieldWithPath("roles").description("Replacement managed role set. `USER` must always be present."),
                                fieldWithPath("roles[]").description("Role name."),
                                fieldWithPath("reason").description("Short operator-supplied explanation for the role change.")
                        ),
                        responseHeaders(commonResponseHeaders()),
                        relaxedResponseFields(
                                fieldWithPath("id").description("Persisted user identifier."),
