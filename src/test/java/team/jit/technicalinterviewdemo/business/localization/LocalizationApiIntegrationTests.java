...
class LocalizationApiIntegrationTests extends AbstractMockMvcIntegrationTest {
    ...
    void listLocalizationsReturnsPaginatedResponse() throws Exception {
        mockMvc.perform(get("/api/localizations").queryParam("page", "0").queryParam("size", "2").queryParam("sort", "messageKey,asc").queryParam("sort", "language,asc")).andExpect(status().isOk()).andExpect(jsonPath("$.content.length()").value(2)).andExpect(jsonPath("$.content[0].messageKey").value("error.book.isbn_duplicate")).andExpect(jsonPath("$.content[0].language").value("de")).andExpect(jsonPath("$.content[0].createdAt").value(endsWith("Z"))).andExpect(jsonPath("$.content[0].updatedAt").value(endsWith("Z"))).andExpect(jsonPath("$.content[1].language").value("en")).andExpect(jsonPath("$.totalElements").value(totalSeededLocalizations())).andExpect(jsonPath("$.totalPages").value(totalPagesForPageSize(2)));
    }
    ...
    void getLocalizationByIdReturnsRequestedMessage() throws Exception {
        mockMvc.perform(get("/api/localizations/{id}", bookNotFoundEs.getId())).andExpect(status().isOk()).andExpect(jsonPath("$.id").value(bookNotFoundEs.getId())).andExpect(jsonPath("$.messageKey").value("error.book.not_found")).andExpect(jsonPath("$.language").value("es")).andExpect(jsonPath("$.messageText").value("No se encontro el libro solicitado.")).andExpect(jsonPath("$.createdAt").value(endsWith("Z"))).andExpect(jsonPath("$.updatedAt").value(endsWith("Z")));
    }

    @Test
    void getLocalizationByIdReturnsNotFoundWhenMissing() throws Exception {
        mockMvc.perform(get("/api/localizations/{id}", 9999)).andExpect(status().isNotFound()).andExpect(jsonPath("$.title").value("Localization Not Found")).andExpect(jsonPath("$.detail").value("Localization with id 9999 was not found.")).andExpect(jsonPath("$.messageKey").value("error.localization.not_found")).andExpect(jsonPath("$.message").value("The requested localization message was not found.")).andExpect(jsonPath("$.language").value("en"));
    }

    @Test
    void listLocalizationsCanFilterByMessageKeyAndLanguage() throws Exception {
        mockMvc.perform(get("/api/localizations").queryParam("messageKey", "error.request.invalid").queryParam("language", "EN")).andExpect(status().isOk()).andExpect(jsonPath("$.content.length()").value(1)).andExpect(jsonPath("$.content[0].id").value(invalidRequestEn.getId())).andExpect(jsonPath("$.content[0].messageKey").value("error.request.invalid")).andExpect(jsonPath("$.content[0].language").value("en")).andExpect(jsonPath("$.content[0].messageText").value("The request is invalid.")).andExpect(jsonPath("$.totalElements").value(1)).andExpect(jsonPath("$.totalPages").value(1));
    }
    ...
    void deleteLocalizationWithoutAuthenticationReturnsUnauthorized() throws Exception {
        mockMvc.perform(delete("/api/localizations/{id}", bookNotFoundEn.getId())).andExpect(status().isUnauthorized());
    }
    ...
    void deleteLocalizationRemovesMessage() throws Exception {
        ...
        mockMvc.perform(get("/api/localizations/{id}", bookNotFoundEn.getId())).andExpect(status().isNotFound()).andExpect(jsonPath("$.title").value("Localization Not Found"));
    }
    ...
    void listLocalizationsCanFilterByLanguage() throws Exception {
        mockMvc.perform(get("/api/localizations").queryParam("language", "es").queryParam("size", "50").queryParam("sort", "messageKey,asc")).andExpect(status().isOk()).andExpect(jsonPath("$.content.length()").value(documentedKeyCount())).andExpect(jsonPath("$.content[0].language").value("es")).andExpect(jsonPath("$.content[0].messageKey").value("error.book.isbn_duplicate")).andExpect(jsonPath("$.content[%d].messageKey".formatted(documentedKeyCount() - 1)).value(lastDocumentedKeySortedByMessageKey())).andExpect(jsonPath("$.totalElements").value(documentedKeyCount())).andExpect(jsonPath("$.totalPages").value(1));
    }
