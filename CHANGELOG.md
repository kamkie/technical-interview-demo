...
- Added `docs/FRONTEND_AI_CONTRACT.md` as the backend-owned source contract, security guidance, approved OpenAPI source reference, and design-constraint handoff for AI agents working in a separate first-party frontend repository.
...
- Fixed the category database invariant so case-variant duplicate category names cannot bypass service-level uniqueness checks.
- Fixed generic database-integrity error wording so non-book constraint failures do not mention books.
- Fixed the auth-failure Prometheus alert expression so protected book writes are included with other protected write endpoints.
...
- Recorded accepted `v2.0.0-RC7` manual-regression evidence, archived the released plan, and advanced the roadmap to post-`2.0` work selection.
...
- Added a `manualRegressionExampleReport` task that writes synthetic manual-regression `report.md`, `report.json`, and `execution-log.ndjson` artifacts under gitignored `temp/`.
