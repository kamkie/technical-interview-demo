...
`.agents/references/releases.md` owns intentional release preparation after implementation is already complete on `main`.

Use this file when the task is to prepare a release commit, tag it, push it, or verify a published release.
Stay in `.agents/references/plan-execution.md`, `.agents/references/execution.md`, or `.agents/references/workflow.md` while work is still local, on a side branch, or in an open PR.
...
- archive released concrete `.agents/plans/PLAN_*.md` files under `.agents/archive/` during post-release cleanup, never archive `.agents/plans/PLAN_TEMPLATE.md` as active work, and stop treating archived plans as active read-set inputs
...
- required specs, contract docs, and maintainer docs are aligned through `.agents/references/documentation.md`
...
- `.agents/references/release-checklist.md`: use when preparing the release metadata commit and annotated tag from local `main`
- `.agents/references/release-artifact-verification.md`: use when pushing a prepared release, monitoring publication, or verifying published artifacts
