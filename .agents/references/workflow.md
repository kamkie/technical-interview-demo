...
`.agents/references/workflow.md` owns branch, worktree, delegation, worker-log, integration, and remote-handoff mechanics.
...
Use `.agents/references/plan-execution.md` for whole-plan execution and `.agents/references/execution.md` for ad hoc or one-milestone work.
...
- keep release sequencing out of workflow execution; use `.agents/references/releases.md` only after implementation is integrated and release is in scope
...
Use `.agents/references/workflow-delegated-plan.md` when one active plan has disjoint worker-safe slices but one coordinator still owns the plan, changelog, final validation, and integration order.
...
Use `.agents/references/workflow-coordinated-plans.md` when separate approved active plans move in parallel and later need one integration pass.
...
`.agents/tmp/workflow/<plan_stem_or_topic>__<worker_name>.md`
...
`.agents/tmp/workflow-local/<plan_stem_or_topic>__<worker_name>.local.md`
...
- final validation from `.agents/references/testing.md`
- final review and documentation alignment using `.agents/references/reviews.md` and `.agents/references/documentation.md`
