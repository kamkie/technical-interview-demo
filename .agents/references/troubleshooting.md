...
| A plan archive or roadmap cleanup conflicts. | Active work, release cleanup, or archive movement disagrees with lifecycle state. | Re-read `.agents/references/releases.md`, `.agents/references/workflow.md`, and the target plan lifecycle; update the active owner artifact once. | Do not leave the same completed plan active in `ROADMAP.md` and archived under `.agents/archive/`. |
