Integrate completed worker output for all merged `Multi-Plan Fanout` branches.

Use `ai/WORKFLOW.md` as coordinator in `Multi-Plan Fanout` mode.
...
If there are no in-scope merged `Multi-Plan Fanout` branches, say so explicitly and stop.
