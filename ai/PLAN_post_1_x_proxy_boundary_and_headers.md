...
- Current behavior:
  ...
  - `src/docs/asciidoc/index.adoc`, `src/test/resources/http/authentication.http`, and `README.md` already say the supported browser contract assumes one public origin via reverse proxy and no CORS support.
  - `README.md`, `SETUP.md`, `src/docs/asciidoc/technical-endpoints.adoc`, and `src/test/resources/http/technical-endpoints.http` already frame `/actuator/prometheus` as trusted deployment scraping rather than internet-public contract.
...
- Current constraints:
  ...
  - the generated docs page under `/docs` is part of the supported contract, so any security-header baseline must not break the generated HTML or its assets
...
- User decisions already locked in the roadmap:
  ...
  - technical endpoints such as Prometheus stay deployment-scoped and privately reachable rather than becoming an application-authenticated API
...
- behavior to preserve
  ...
  - docs remain honest about same-site-only browser support and private technical-endpoint exposure
...
## User Validation
- Start the app in a production-like configuration and verify the root overview still reports the documented session posture plus any new proxy/header posture fields added by this slice.
- Request `/api/session`, `/docs`, `/actuator/info`, and `/actuator/health/readiness` and confirm the documented security headers are present where intended.
- Verify `/actuator/prometheus` still works for trusted local inspection or deliberate port-forward usage, while the docs clearly keep it out of the internet-public contract.
- Confirm the session contract still returns relative login/logout/account paths and still does not imply cross-origin browser support.
