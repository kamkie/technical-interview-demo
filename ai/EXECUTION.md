...
`ai/EXECUTION.md` owns ad hoc task execution and individual plan-milestone execution.
...
Use `ai/PLAN_EXECUTION.md` when the user asks to execute a whole active plan across milestones.
Use `ai/WORKFLOW.md` only when branch, worktree, delegation, worker-log, or integration mechanics are needed.
Use `ai/PLANNING.md` when the task is not clear enough to execute without first creating or revising a plan.
...
- `ai/DOCUMENTATION.md` when artifact routing or AI-document maintenance is involved
- `ai/TESTING.md` for validation scope
- `ai/REVIEWS.md` before finalizing
- `ai/WORKFLOW.md` when worktrees, delegation, worker logs, or integration handoff are in scope
...
10. Commit when the task or milestone is complete and the active workflow expects a checkpoint commit. Include a `Trigger: <category> - <source>` body line using `plan milestone` for one named plan milestone, `ad hoc task` for a direct user request, or `task-library starter` for a reusable task from `ai/TASK_LIBRARY.md`.
...
- setup and environment changes route through `SETUP.md` and, when AI command guidance changes, `ai/ENVIRONMENT_QUICK_REF.md`
- AI-guidance changes route to the owning `ai/` guide through `ai/DOCUMENTATION.md`
- required validation that cannot run must be recorded explicitly
- never edit files reserved to another worker or coordinator without first resolving ownership through `ai/WORKFLOW.md`
