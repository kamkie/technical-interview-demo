...
Additional validation-unblocker artifact:
...
- 2026-05-06: `./build.ps1 build gatlingBenchmark --no-daemon` initially failed in `compileExternalTestJava` because Error Prone `StringConcatToTextBlock` threw an unhandled exception in `ExternalSessionSupport.isJdbcConfigured`.
- 2026-05-06: Applied a behavior-preserving local-variable refactor in `ExternalSessionSupport.isJdbcConfigured`; `./build.ps1 compileExternalTestJava` passed afterward.
- 2026-05-06: `./build.ps1 build gatlingBenchmark --no-daemon` reran and failed at `gatlingBenchmark` because request `oauth2-github-redirect` regressed (`p95 45ms > 14ms`).
- 2026-05-06: `./build.ps1 build --no-daemon` passed, including tests, REST Docs generation, Docker image build, Trivy dependency and image scans, SBOM generation, PMD, SpotBugs, Spotless, and coverage verification.
- 2026-05-06: `./build.ps1 gatlingBenchmark --no-daemon` reran and failed again because request `oauth2-github-redirect` regressed (`p95 36ms > 14ms`).
