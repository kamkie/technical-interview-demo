...
Load it only after `ai/RELEASES.md` release preconditions are satisfied and the task is preparing a release from local `main`.
...
9. Move the executed `ai/plans/PLAN_*.md` file to `ai/archive/` and update moved-path references in the same change.
