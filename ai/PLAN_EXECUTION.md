...
Use `ai/RELEASES.md` only for explicit release work after implementation is integrated.
...
- `ai/DOCUMENTATION.md` when artifact routing is part of the work
- `ai/TESTING.md` for validation scope
- `ai/REVIEWS.md` before finalizing a milestone
- `ai/WORKFLOW.md` only when delegation, worktrees, worker logs, or integration mechanics are in scope
...
6. If validation fails, load `ai/references/TROUBLESHOOTING.md` before choosing a recovery path.
7. Review the validated diff using `ai/REVIEWS.md`.
...
10. Commit the completed milestone before starting the next one. Include a `Trigger: <category> - <source>` body line, normally `Trigger: milestone - <plan path> <milestone name>`; use `plan implementation` only for a commit that intentionally covers a whole active plan checkpoint rather than one milestone.
...
- follow `ai/WORKFLOW.md` for worker logs and integration notes only when work has actually been split
...
- required validation from `ai/TESTING.md` passed, or any inability to run it is recorded
