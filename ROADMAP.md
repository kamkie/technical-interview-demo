...
| --- | --- | --- | --- |
| In Progress | Repo task skill migration | `ai/plans/PLAN_repo_task_skill_migration.md` | Replace the former `ai/TASK_LIBRARY.md` catalog with the low-context `repo-task` skill described by the plan's inline spec; implementation must update live AI guidance so the skill is the single task-starter source of truth |
